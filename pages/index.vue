...
<template>
  <!-- Hero Section -->
  <section class="mb-12">
    <div class="hero-section bg-gradient-to-r from-itsy-blue to-itsy-blue/80 rounded-xl p-6 md:p-8 mb-12 text-itsy-white relative overflow-hidden">
      <div class="hero-content md:max-w-2/3 relative z-10">
        <h1 class="text-3xl md:text-4xl font-bold mb-4 text-itsy-white">
          <span class="text-itsy-yellow">Quick</span> news summaries without the noise
        </h1>
        <p class="text-itsy-white text-lg mb-6">
          Concise one-minute reads from trusted sources. Stay informed without the sensationalism.
        </p>
        <NuxtLink to="/news" class="inline-block bg-itsy-yellow hover:bg-itsy-yellow/90 text-itsy-black font-medium px-6 py-3 rounded-md transition-colors duration-300">
          Browse All News
        </NuxtLink>
      </div>
    ...
    <div class="mb-8">
      ...
      <div v-else class="bg-itsy-white p-8 rounded-lg text-center">
  ...
  <section class="mb-12">
    <div class="flex justify-between items-center mb-6 pb-2 border-b border-itsy-black/10">
      ...
      <NuxtLink to="/news" class="text-itsy-red hover:text-itsy-red/80 font-medium transition-colors flex items-center">
        View All
  ...
  <section>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      <NuxtLink 
        ...
        class="group bg-white border border-itsy-white shadow-sm hover:shadow-md rounded-lg p-6 text-center transition-all duration-300 hover:-translate-y-1 flex flex-col items-center justify-center"
      >
        <div class="category-icon bg-itsy-teal/20 text-itsy-blue p-3 rounded-full mb-3 text-2xl transition-colors group-hover:bg-itsy-blue group-hover:text-itsy-white">
          {{ category.icon || '📰' }}
        </div>
        <h3 class="font-bold text-lg text-neutral-800">{{ category.name }}</h3>
      </NuxtLink>
    </div>
  </section>
